...
const prompt = require('prompt-sync')(); // я выхожу с карточки назад и захожу в +1, а можно изначально хранить ссылку на следующий мак и после получения цены сразу переходить на новую ссылку
// creating date
var today = new Date();
...
var defaultTime = 500;              // 550 w/o doubler // until 14.09 == 750
// variables for excel
const qwerty = 'QWERTY';
const color = [
    'silber', 
    ...
    */
        ...
        while (year < 2020) {
            try {
                ...
                for (; anzeigeCounter < anzeigen + 1; anzeigeCounter++) { // here you can limit number of anzeigen 'for( ; anzeigeCounter < anzeigen + 1; anzeigeCounter++){
                    ...
                    [mainPageButton] = await page.$x('//*[@id="ry"]/body/rebuy-app/div/div/ry-product-list/ry-product-list-electronic/div[1]/div/div/section/ry-product[' + anzeigeCounter + ']/a/button');
                    ...
                    try {
                        if (btnTxtKA != ' Kein Ankauf ') {
                            ...
                            [modelText] = await page.$x('//*[@id="ry"]/body/rebuy-app/div/div/ry-product-list/ry-product-list-electronic/div[1]/div/div/section/ry-product[' + anzeigeCounter + ']/a/p');
                            ...
                            for (; questionDiv < 8; questionDiv++) { // 29.03.22 // 27.05.22
                                if (questionDiv < 6){
                                    [survey] = await page.$x('//*[@id="ry"]/body/rebuy-app/div/div/ry-product-detail-view/div/div/ry-product-gradable/div/div[1]/div/div/div[' + questionDiv + ']/ry-grading2-view/ry-grading-title-text/div[3]/ry-grading2-radio/div/div[1]/div');
                                }
                                else{
                                    [survey] = await page.$x('//*[@id="ry"]/body/rebuy-app/div/div/ry-product-detail-view/div/div/ry-product-gradable/div/div[1]/div/div/div[' + questionDiv + ']/ry-grading2-view/ry-grading-title-text/div[3]/ry-grading2-radio/div/div/div');
                                }
                            ...
                            await delay(defaultTime * 1.5); // it is necessary       --------------
                            [bestPrice] = await page.$x('//*[@id="ry"]/body/rebuy-app/div/div/ry-product-detail-view/div/div/ry-product-gradable/div/div[2]/ry-product-gradable-price-box/div/div/div/div[2]/ry-grading-price-summary/div/div[2]'); //try get price if 0 wait time
    ...
    async function old13() {
        try {
            ...
            for (; anzeigeCounter < anzeigen + 1; anzeigeCounter++) { // here you can limit number of anzeigen 'for( ; anzeigeCounter < anzeigen + 1; anzeigeCounter++){
                ...
                btnTxtKA = await btnTxt.jsonValue();
                try {
                    if (btnTxtKA != ' Kein Ankauf ') { // 27.02.23 added space in the beginning
                        // getting model text (DO NOT PRINT in the console)
                        [modelText] = await page.$x('//*[@id="ry"]/body/rebuy-app/div/div/ry-product-list/ry-product-list-electronic/div[1]/div/div/section/ry-product[' + anzeigeCounter + ']/a/p');
    ...
    async function old16() {
        ...
        try {
            ...
            nummer = await anzeigeNummer.getProperty('textContent');
            anzeigen = await nummer.jsonValue();
            anzeigen = parseInt(anzeigen.substr(23, 25));
            ...
            for (; anzeigeCounter < anzeigen + 1; anzeigeCounter++) { //  for( ; anzeigeCounter < anzeigen + 1; anzeigeCounter++){
                ...
                if (btnTxtKA != ' Kein Ankauf ') {
                    ...
                    // getting the price value
                    await delay(defaultTime * 1.5); // it is necessary       --------------
                    [bestPrice] = await page.$x('//*[@id="ry"]/body/rebuy-app/div/div/ry-product-detail-view/div/div/ry-product-gradable/div/div[2]/ry-product-gradable-price-box/div/div/div/div[2]/ry-grading-price-summary/div/div[2]');
                    value = await bestPrice.getProperty('textContent');
                    priceWN = await value.jsonValue();
                    priceWN = parseInt(priceWN);
